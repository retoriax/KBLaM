...
parser.add_argument("--val_dataset", type=str, default=None, help="Path to the validation dataset JSON file")
...
def create_custom_progress_bar(
    ...
    columns.extend(
        [
            ...
            TaskProgressColumn(),
            TextColumn("[bold yellow]Train Loss: {task.fields[train_loss]:.4f} | Val Loss: {task.fields[val_loss]:.4f} | Train Acc: {task.fields[train_accuracy]:.2f} | Val Acc: {task.fields[val_accuracy]:.2f}", justify="right"),
        ]
    )
...
def get_batch(
    ...
    with torch.autograd.no_grad():
        ...
        tokenizer_output = tokenizer(input_strs, return_tensors="pt", padding=True, return_attention_mask=True).to(device)
...
class Trainer:
    ...
    def train(
        ...
        # Warn if no validation dataset provided before training loop
        if val_dataset is None:
            self.logger.warning("No validation dataset provided. Validation loss will not be computed.")

        with create_custom_progress_bar(console=console, disable=not self.accelerator.is_main_process) as pbar:
            task = pbar.add_task("Training", total=self.num_steps, train_loss=100, val_loss=0, train_accuracy=0.0, val_accuracy=0.0)
            for step in range(start_step, self.num_steps, 1):
                ...
                for a_step in range(start_accum_step, end_accum_step):
                    try:
                        ...
                        # Failsafe for attention_masks
                        if attention_masks is None:
                            self.logger.warning("attention_masks is None – falling back to all-ones attention mask.")
                        ...
                        with torch.no_grad():
                            ...
                            mask = shift_labels >= 0
                            step_correct += int((preds == shift_labels)[mask].sum().item())
                            step_total += int(mask.sum().item())
                        ...
                        self.logger.error(f"CUDA OOM at step {step}, accum step {a_step}: {e}")
                ...
                train_accuracy = step_correct / step_total if step_total > 0 else 0.0

                # Only log from main process
                if self.accelerator.is_main_process:
                    log_msg = f"step: {step}, loss: {avg_loss}"
                    if step % 1 == 0 and val_dataset is not None and len(val_losses) > 0:
                        log_msg += f", val_loss: {val_losses[-1]}"
                    self.logger.info(log_msg)
                    wandb.log({'train_loss': np.mean(losses), 'train_accuracy': train_accuracy}, step=step, commit=False)
                    train_accuracies.append(train_accuracy)
                    train_losses.append(avg_loss)
                    pbar.update(task, advance=1, train_loss=avg_loss, train_accuracy=train_accuracy)

                # Validation step
                if step % 1 == 0:
                    if val_dataset is None:
                        self.logger.warning("⚠️ Skipping validation step because val_dataset is None.")
                    else:
                        # synchronize all processes before validation
                        self.accelerator.wait_for_everyone()
                        with torch.no_grad():
                            val_input_ids, val_attention_masks, val_labels, val_batch_indices = self._get_batch(
                                val_dataset,
                                self.tokenizer,
                                self.device,
                                B=batch_size,
                            ...
                            if val_attention_masks is None:
                                self.logger.warning("val_attention_masks is None – falling back to all-ones attention mask.")
                                val_attention_masks = torch.ones_like(val_input_ids)
                            elif isinstance(val_attention_masks, torch.Tensor) and val_attention_masks.device != val_input_ids.device:
                                val_attention_masks = val_attention_masks.to(val_input_ids.device)
                            val_kb_embedding = self.kbretriever.get_key_embeddings(
                                val_batch_indices, len(val_input_ids), step, self.kb_size
                            )
                            val_out = self.model(
                                input_ids=val_input_ids,
                                attention_mask=val_attention_masks,
                                kb_kvs=val_kb_embedding,
                                output_attentions=False,
                                kb_config=kb_config,
                            )
                            val_logits = val_out["logits"]
                            shift_val_logits = val_logits[..., :-1, :].contiguous()
                            shift_val_labels = val_labels[..., 1:].contiguous()
                            with torch.no_grad():
                                val_preds = shift_val_logits.argmax(dim=-1)
                                val_mask = shift_val_labels >= 0
                                val_correct = int((val_preds == shift_val_labels)[val_mask].sum().item())
                                val_total = int(val_mask.sum().item())
                                val_accuracy = val_correct / val_total if val_total > 0 else 0.0
                            val_weights = (shift_val_labels > 0).sum(-1, keepdim=True).expand(-1, shift_val_labels.shape[1]).contiguous()
                            model_config = (
                                self.model.config
                                if not isinstance(self.model, DistributedDataParallel)
                                else self.model.module.config
                            )
                            shift_val_logits = shift_val_logits.view(-1, model_config.vocab_size)
                            shift_val_labels = shift_val_labels.view(-1).to(shift_val_logits.device)
                            val_weights = val_weights.view(-1)
                            loss_fct = CrossEntropyLoss(reduction="none")
                            raw_val_loss = (loss_fct(shift_val_logits, shift_val_labels) * val_weights.max() / val_weights).mean()
                            # gather and average across GPUs
                            val_loss_tensor = torch.tensor(raw_val_loss, device=self.device)
                            all_val_losses = self.accelerator.gather(val_loss_tensor)
                        if self.accelerator.is_main_process:
                            avg_val_loss = all_val_losses.mean().item()
                            wandb.log({'val_loss': avg_val_loss, 'val_accuracy': val_accuracy}, step=step)
                            val_accuracies.append(val_accuracy)
                            val_losses.append(avg_val_loss)
                            pbar.update(task, val_loss=avg_val_loss, val_accuracy=val_accuracy)
                        # clean up
                        del val_input_ids, val_attention_masks, val_labels, val_logits, val_kb_embedding, shift_val_logits, shift_val_labels, val_weights
                        import gc
                        gc.collect()
                        torch.cuda.empty_cache()
...
def main():
    ...
    if Accelerator().is_main_process:
        ...
        webbrowser.open(wandb.run.get_url())
    ...
    if use_extended_qa:
        with open(os.path.join(dataset_dir, f"{dataset_name}_augmented.json"), "r", encoding="utf-8") as file:
            dataset = json.load(file)
    else:
        with open(os.path.join(dataset_dir, f"{dataset_name}.json"), "r", encoding="utf-8") as file:
            dataset = json.load(file)
    ...
    if args.val_dataset is not None:
        val_dataset_path = os.path.join(dataset_dir, f"{args.val_dataset}.json")
