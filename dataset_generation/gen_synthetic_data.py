...
def construct_prompts(entity: DataPoint) -> tuple[str, str, str]:
    """Erzeugt eine deutschsprachige Frage-Antwort-Paar mit richtigem Artikel."""
    mapping = {
    ...
    begriff, artikel = mapping.get(entity.description_type, (entity.description_type, "die"))
    Q = f"Was ist {artikel} {begriff} von {entity.name}?"
    A = f"{artikel.capitalize()} {begriff} von {entity.name} ist {entity.description}."
    key_string = f"{artikel} {begriff} von {entity.name}"
