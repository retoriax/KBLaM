...
class KblamLlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        ...
        if rope_scaling is None:
            ...
            if rope_type == "llama3":
                # TODO: Hier ein passendes RotaryEmbedding-Objekt verwenden,
                # bis dahin fallback:
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                logger.warning_once("Using fallback rotary embedding for llama3 rope_type.")
                ...
                raise ValueError(f"Unknown RoPE scaling type or rope_type: {rope_scaling}")
        else:
            raise ValueError(f"Invalid rope_scaling config: {rope_scaling}")
        ...
        if kb_kvs is not None:
            if self.layer_idx % kb_layer_frequency == 0:
                ...
                if attention_mask is None:
                    attention_mask = torch.ones((bsz, q_len), dtype=torch.bool, device=hidden_states.device)
                ...
                padding_mask = attention_mask[:, :, :, :1] < 0  # Broadcasting-safe
                kb_atten_mask = (
                    padding_mask * PADDING_VALUE + (~padding_mask) * kb_atten_mask
                )
                # Ensure attention_mask is the correct shape before concatenation
                if attention_mask.size(-1) != 1:
                    attention_mask = attention_mask.expand(bsz, 1, q_len, attention_mask.size(-1))
